...
| 문제 | 영향 |
|------|------|
| **비결정적 출력** | 같은 입력에 다른 결과 → 감사 불가 |
| **판정 근거 불명확** | "왜 리젝?" 설명 어려움 |
| **정책 변경 어려움** | 프롬프트 수정 시 부작용 예측 불가 |
...
|------|------|
| **결정론적** | 같은 점수 → 항상 같은 판정 |
| **감사 가능** | Python 코드로 판정 로직 명시 |
| **테스트 가능** | 단위 테스트로 정책 검증 |
| **LLM 비의존** | 평가는 LLM, 판정은 SOP 분리 |
...
| 구분 | 역할 | 형태 | 예시 | 상태 |
|------|------|------|------|------|
| **SOP** | 의사결정 절차 | Python 클래스 | 점수 판정 로직 | ✅ 구현됨 |
| **Tool** | 실행 래퍼 | Python 함수 | 에이전트 호출 | ✅ 구현됨 |
| **Skill** | 지식/능력 정의 | Markdown (SKILL.md) | 번역 가이드라인 | 🔮 향후 기능 |
...
| **실행 주체** | Python 코드 | LLM |
| **결정론적** | ✅ 항상 동일 결과 | ❌ 가변적 |
| **용도** | 정책/판정 로직 | 도메인 지식/가이드라인 |
| **테스트** | 단위 테스트 가능 | 프롬프트 평가 필요 |

> **Skills (향후 기능)**: 도메인별 번역 가이드라인, 스타일 규칙 등을 Markdown으로 정의하여 에이전트에 주입하는 지식 패키지. 현재는 프롬프트에 직접 포함되어 있으며, 향후 재사용 가능한 Skill 모듈로 분리 예정.
